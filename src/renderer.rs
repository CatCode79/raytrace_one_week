...
impl Renderer {
    pub fn new(window: &Window) -> Result<Self, String> {
        ...
        let surface_config =
            create_surface_config(&surface, &adapter, window)?;
    ...
    pub fn resize(&mut self, width: NonZeroU16, height: NonZeroU16) {
        ...
        if self.surface_config.width as u16 != width || self.surface_config.height as u16 != height {
            ...
            self.result_texture =
                TextureHandler::new(&self.device, new_size);
    ...
    pub fn update(&mut self, buffer: &Buffer) -> Result<(), String> {
        ...
        let temp_buf = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
...
fn create_surface_config(
    ...
    if window.inner_width() == 0 {
        return Err("Impossible to create a surface configuration with zero width size".to_string());
    }
    if window.inner_height() == 0 {
        return Err("Impossible to create a surface configuration with zero height size".to_string());
    }
